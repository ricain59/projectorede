	...
	public void init() { 
		while(true) { 
			try { 
				...
				if(request.requestType().equals(HTTPMethod.GET)) { // reply to a GET request
					...
					// if reply is OK
					if(reply.getHTTPResponse().equals(HTTPResponce.OK)) { // the files needs to be sent
						// open requested file
				...
				else if(request.requestType().equals(HTTPMethod.PUT)) { // PUT request
					System.out.println("PUT operation");
					
					//TODO: Implement this
					
					toBrowser.write((HTTPResponce.ERROR.toString() + "\r\n\r\n").getBytes());
				}
				else if(request.requestType().equals(HTTPMethod.POST)) { // POST request
